...
class_indexing = fileName_id("train")
def read_data(path, config, usage="train"):
    # 从目录中读取图像的源数据集。
    dataset = ds.ImageFolderDataset(path, class_indexing=class_indexing)
    ...
    random_crop_decode_resize_op = vision.RandomCropDecodeResize((cfg.HEIGHT, cfg.WIDTH), (1.0, 1.0), (1.0, 1.0),
                                                                 max_attempts=100)
...
print('通道数/图像长/宽：', data_next['image'][0, ...].shape)  # 一共2388类，用0-2387的数字表达类别。
